...
[DESIGN-KIT-PLAN.md](DESIGN-KIT-PLAN.md) for the screens still waiting on it, and
...
| **`JsonB`** (ADR-0017) | JSONB on PostgreSQL, JSON on SQLite, one declaration. Documents are stored binary — indexable, containment-queryable, parsed once — rather than as text reparsed on every access. |
...
| **SQLite for speed, PostgreSQL for truth** | The fast suite runs on SQLite; schema, migrations and partitioning are verified against real PostgreSQL in `test_migrations.py` and `test_partitions.py`, skipped only when no server is reachable. |
...
| `uq_{order,job}_events_*_sequence`, `uq_refunds_payment_id_sequence` | Ambiguous ordering in the append-only histories. `sequence` is the only dependable ordering — `created_at` has second granularity on SQLite and UUIDv7 only orders to the millisecond — so the constraint is what makes it dependable rather than merely intended |
...
[DESIGN-KIT-PLAN.md](DESIGN-KIT-PLAN.md) enumerates the missing backend screen by
screen. Expressed as schema:
...
**Telemetry retention is disabled by default** (`telemetry_retention_days = 0`).
Dropping a partition is irreversible, and until rollups exist the raw samples are the
only copy of what the farm measured. It is enabled in the same change that starts
summarising them, not before.

**SQLite does not enforce foreign keys, so most of the fast suite does not test
them.** PostgreSQL always does; SQLite ignores them unless `PRAGMA foreign_keys=ON`
is issued per connection. Tests that assert on a constraint request the
`enforce_foreign_keys` fixture and get the real behaviour — the `RESTRICT` that
protects a model an order still needs is covered that way. Turning it on for the
whole suite fails 66 existing tests that build a job or a line against a fabricated
parent id; those are worth fixing, and doing it inside an unrelated change would
bury both.
...
| # | Task |
|---|---|
| DB-1 | `settings` + `settings_audit` (Slice D). Reads fall back to the code default so an empty table behaves exactly as today; `RateSnapshot` resolution stays at the edge so ADR-0002 purity holds. |
| DB-2 | `metric_rollups` over `telemetry_samples`, then enable `telemetry_retention_days` in the same change (Slice G). |
| DB-3 | Account tables: addresses, payment methods, notification preferences, reset codes (Slice E). |
| DB-4 | Catalogue tables — categories, tags, ratings — joining measured time and price from completed jobs onto the `model_assets` that now exist (Slice F). |
| DB-5 | Post-production, QC, fulfilment and procurement (Slice H / Phases 5–6). |
| DB-6 | Journal (Slice I). |
