...
**Fifty-three foreign keys, each with a deliberate delete rule** — 28 `CASCADE`,
17 `SET NULL`, 8 `RESTRICT`. The enumeration is
`backend/tests/test_referential_integrity.py` rather than the list below: it names
all fifty-three as `table.column`, fails if a fifty-fourth is added without somebody
