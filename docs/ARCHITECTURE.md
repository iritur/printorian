...
| **Observability** | **Logging works.** Structured logs — JSON in production, plain text elsewhere — every line carrying the correlation id of the request that produced it, taken from the caller's `X-Request-ID` when there is one so a trace begun at the proxy continues, and echoed back so a person reporting a problem can quote it (`api.middleware`). One access line per request with its duration, which is the only signal here that can see a slow *request* rather than a slow *query*. **Health works, and the four endpoints answer four different questions.** `/health` touches nothing. `/health/ready` names each dependency separately so an outage names its own cause: `database`, `telemetry_partitions`, `assignment_records`, `printers_listing`, `materials_listing`, `wal_archiving`, and — only where a relay is configured at all — `event_relay`. Only `database` can come back `failed`, and only that returns 503; the rest report `degraded` at 200, because a broken backup, a quiet live board or a table that has outgrown its shape must not be turned into a broken farm. `assignment_records` reports a *threshold* rather than a fault, which is what makes it different from `wal_archiving` beside it: the table ADR-0018 deliberately left unpartitioned, measured out of the catalogue against the 10M-row / 20 GiB trigger, and once crossed it stays lit until the table is split rather than clearing when a condition passes. The two `*_listing` checks are the third shape again: the listings DATABASE-REVIEW §9 left unpaged, counted against a 500-row trigger by a count that stops at the trigger so the probe cannot become the cost, and *clearing on their own* — a set that can shrink, unlike a table that has already been written. **Drivers are deliberately not among them** — the pool of live printer connections belongs to the *worker* process (`workers/drivers.py`, kept alive between passes so a fifty-machine farm does not reconnect every tick), so the API has no connection state of its own to report and a readiness check here would have to invent one. **They are reported at `/health/workers` instead**, which is where the same argument leads rather than where it stops: the worker publishes what it observed down the channel the heartbeat already uses (`core.driver_health`), and the API reports it without pretending to own it. Per printer and never as a count, with the error code and when the state began — an outage that cannot name its own cause is one somebody has to go and find. A printer the worker named and then stopped publishing readings for reports `unknown` rather than `ok` (ADR-0007), which is why the roster outlives the readings it names. **A driver that is down never changes a status code**: not readiness, where an unreachable printer would take the whole API out of rotation, and not `/health/workers` either, where it would sit permanently red on any farm with a machine switched off and destroy the signal the endpoint exists for. Only the loops decide that, and an alert about a driver keys on the body. `/health/workers` covers the seven sweeps by name, from a beat each records at the *end* of a pass, and is kept out of readiness on purpose: a wedged sweep is a reason to alert, not to roll a release back. **`/health/printing` is the fourth, and the only one here whose consumer is neither a screen nor a container runtime**: `deploy/reboot-guard.sh` on the host asks it before `unattended-upgrades` or a UPS shutdown cuts power, so the status code *is* the verdict and it fails closed — 200 only for a reading that was taken and came back with nothing on a machine, and 503 for a farm that is busy and for a database that could not be read alike. Those two share a code and differ in the body on purpose, so that a guard simple enough to be trusted defers without parsing anything; the unreadable case answers `in_flight: null` and never three zeros, because three zeros read as an idle farm on the one path where that authorises pulling the plug (ADR-0007). DISPATCHING vetoes and ASSIGNED does not — a half-sent plate file needs a person, while an assignment is a plan the planner re-makes after a restart, and a job wedged in ASSIGNED would hold the veto on for ever and the host would never patch again. **The trigger half is deliberately absent**: nothing in this repository reboots anything, and `deploy/systemd/README.md` says what would have to be shown on a real host first. **Metrics have started.** `GET /metrics` serves a Prometheus text exposition (`core.metrics`, `prometheus-client`), unauthenticated like the probes, built on a **registry per scrape** rather than the process-global default so two `create_app` instances in one test session cannot accumulate each other's series. It carries **three** of INFRASTRUCTURE §5's ten domain metrics — the three whose data the API process already measures: `printorian_printers_offline{printer,brand}`, `printorian_telemetry_partition_months_ahead`, `printorian_wal_archive_failures_total`. ADR-0007 is structural here rather than remembered: `observe(value=None)` sets nothing, so a reading nobody took is an **absent series** and never a zero — a driver reporting `unknown` produces no sample, an unpublished roster produces a family with no samples, and a database that cannot be reached leaves both DB-backed series out while the endpoint still answers 200 with what is known. **No money on this surface**, and that is a decision rather than an oversight: `printorian_sla_credit_accrued_rub` waits until the scrape has an identity, because `VIEW_FINANCIALS` cannot be checked on a caller there is none of. Still missing, and none of it is close: the worker exporter, the four remaining database-backed collectors, the backup and restore-drill stamps (no machine-readable marker is written today), the scrape-auth decision behind the money series, and the whole VictoriaMetrics/Grafana/Alertmanager host stack. Note the collision before concluding otherwise: `GET /fleet/metrics` and `/fleet/metrics/{printer_id}` are the *farm's* measured occupancy behind `VIEW_PRODUCTION`, not an instrumentation scrape. Stage 5 |
