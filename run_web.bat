@echo off
REM ---------------------------------------------------------------------------
REM  Printorian - storefront (web)
REM
REM  Brings up everything the browser app needs and opens it:
REM    Postgres + Redis  ->  API on :8000  ->  workers  ->  Vite on :5173
REM
...
echo [5/5] Storefront on port 5173...
curl -s -m 2 -o NUL http://127.0.0.1:5173/
if errorlevel 1 (
    start "Printorian Web" /d "%~dp0frontend" cmd /k npm run dev --workspace @printorian/web
) else (
    echo     already running - reusing it
)
